...
//                std::cout << "P0 h:" <<  header << "L:" << line << " FIN \n\n"; 
...
std::vector<Sequence> Sequence::read(std::vector<std::string> header, std::vector<std::string> line)
{
        std::vector<Sequence> sequences;
//        std::ifstream infile(fileName);
//        std::string line;
...
//                std::getline(infile, header);
  //              std::getline(infile, line, '>');
		...
		std::cout << "h:" <<  header[i] << "L:" << line[i] << " FIN \n\n"; 
                sequences.push_back(Sequence(header[i], line[i]));
...
//std::cout <<  "obteniendo de los threads\n" ;
