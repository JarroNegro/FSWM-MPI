...
Seed::Seed(int32_t weight, int32_t dontCare, std::vector<std::string> pattern )
{
        this->weight=weight;
        this->dontCare=dontCare;
        this->length=weight+dontCare;
...
void Seed::generatePattern()
{
	...
	patterns=pattern_obj->GetPattern();

	//std::string pattern=pattern_obj->GetBestPattern(0);

	//std::cout<< pattern << " " << weight << " " << dontCare << " \n";
...
void Seed::usePattern(std::vector<std::string> pattern2){
  matchPos.clear();
dontCarePos.clear();
        pattern* pattern_obj;
        pattern_obj = new pattern(NULL, NULL, 1, weight + dontCare, weight, 10000, 10000, 10000, 0.75, 0.25);
        pattern_obj->ImproveSecure();
        pattern_obj->Improve(5000);
	pattern_obj->SetPattern(pattern2);
...
std::vector<std::string> Seed::getPatterns(){
return patterns;
}


void Seed::setPatterns(std::vector<std::string> patterns){
